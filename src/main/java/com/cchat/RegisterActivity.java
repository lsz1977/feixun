...
//        doAction
        doAction(new DAction(ActionProtocol.REGISTER, mAccount, mPassword), new ADWaitDialogListener(this) {
            @Override
...
//                finishAll();
                ...
                CommonUtils.showToast(RegisterActivity.this, (String)values[0] +" code:"+ (int)values[1]);
                if (code < 0)
                    return;
                setResult(0, new Intent().putExtra("register", new RegisterData(mAccount, mPassword)));
                finish();
            }
